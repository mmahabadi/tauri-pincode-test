...
function KeyPad({ onKeyPress, onDelete, disabled }: PropTypes) {
  return (
    <div
      data-testid="KeyPad"
      className="grid grid-cols-3 gap-4 p-4 justify-center"
    >
      {Array.from(Array(9).keys()).map((number) => (
        <div key={"key" + number} className="col-span-1 flex justify-center">
          <CircleButton
            onClick={() => onKeyPress(number + 1)}
            text={number + 1 + ""}
            disabled={disabled}
          />
        </div>
